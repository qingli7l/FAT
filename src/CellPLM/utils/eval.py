...
def imputation_eval(pred_labels, true_labels, dim=1):
    ...
    for i in range(true_labels.shape[dim]):
        ...
        pred_nz = pred_nz.cpu().numpy()
        spearman_corr, _ = st.spearmanr(true_nz, pred_nz)
        spcorr.append(spearman_corr)
        data_range = max(true_nz.max(), pred_nz.max()) - min(true_nz.min(), pred_nz.min())
        ssim_.append(ssim(true_nz, pred_nz, data_range=data_range))
        m = 0.5 * (true_nz + pred_nz)
        js.append(0.5 * (entropy(true_nz, m) + entropy(pred_nz, m)))
        pcc.append(np.corrcoef(true_nz, pred_nz)[0, 1])
