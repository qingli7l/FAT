...
def load_pretrain(
    ...
    config_path = os.path.join('ckpt/', f'{pretrain_prefix}.config.json')
    ckpt_path = os.path.join('ckpt/', f'{pretrain_prefix}.best.ckpt')
        ...
        if self.fitted:
            ...
            if hvg > 0:
                if hvg < adata.shape[1]:
                    sc.pp.highly_variable_genes(adata, n_top_genes=hvg, subset=True, flavor='seurat_v3') # subset：默认是false，只是返回高可变基因，否则就原位替换
                else:
